...
contract RuleProcessingAccountMaxTradeSizeMultiTest is RuleProcessingInvariantCommon {
    ...
    function setUp() public {
        ...
        for (uint j; j < AMOUNT_MANAGERS; j++) {
            ...
            RuleProcessingAccountMaxTradeSizeActorManager actorManager = new RuleProcessingAccountMaxTradeSizeActorManager(tempActors, address(amm), address(testCoin));
            switchToRuleAdmin();
     ...
     * account found in the AccountMaxTradeSize applied for the asset.
     */
     ...
     * tags of the account found in the MaxTradeSize applied for the asset.
     */
    function invariant_amountBoughtCanNeverExceedTheMostRestrictiveTagBuy() public view {
        for (uint j; j < actors.length; j++) {
            for (uint i; i < actors[j].length; i++) {
                // the more restrictive tag
                if (i % 3 == 0)

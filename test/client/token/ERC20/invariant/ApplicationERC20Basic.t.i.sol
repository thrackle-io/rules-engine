 ...
 */
contract ApplicationERC20BasicInvariantTest is TestCommonFoundry {
    address msgSender;
    uint256 value;
    uint256 amount;
    address target;
    ...
    function invariant_ERC20external_userBalancesLessThanTotalSupply() public view {
        uint256 sumBalances = applicationCoin.balanceOf(address(this)) +
            applicationCoin.balanceOf(USER1) +
            applicationCoin.balanceOf(USER2) +
            applicationCoin.balanceOf(USER3);
        assertLe(
            sumBalances,
            applicationCoin.totalSupply(),
    ...
    function invariant_ERC20external_zeroAddressBalance() public view {
        assertEq(
            applicationCoin.balanceOf(address(0)),
            0,
    ...
    function invariant_ERC20external_transferFromToZeroAddress() public {
        ...
        bool r = applicationCoin.transferFrom(
            msg.sender,
            address(0),
            value % (maxValue + 1)
        );
    ...
    function invariant_ERC20external_setAllowance() public {
        ...
        assertTrue(r);
        assertEq(
            applicationCoin.allowance(address(this), target),
            amount
        );
    }

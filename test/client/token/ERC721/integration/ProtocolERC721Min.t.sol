...
contract ProtocolERC721MinTest is TestCommonFoundry, DummyNFTAMM {
    ...
    function testERC721_ZeroAddressChecksERC721() public {
        ...
        vm.expectRevert();
        minimalNFT.connectHandlerToToken(address(0));

        /// test both address checks in constructor
        applicationNFTHandler = _createERC721HandlerDiamond();
       
        vm.expectRevert();
        ERC721HandlerMainFacet(address(applicationNFTHandler)).initialize(address(0x0), address(applicationAppManager), address(minimalNFT));
        vm.expectRevert();
        ERC721HandlerMainFacet(address(applicationNFTHandler)).initialize(address(ruleProcessor), address(0x0), address(minimalNFT));
        vm.expectRevert();
    ...
    function testERC721_AccountMinMaxTokenBalanceRule() public {
        ...
        minimalNFT.transferFrom(appAdministrator, rich_user, 0);
        minimalNFT.transferFrom(appAdministrator, rich_user, 1);
        assertEq(minimalNFT.balanceOf(rich_user), 2);

        ///transfer tokenId 3 and 4 to user1
        minimalNFT.transferFrom(appAdministrator, user1, 3);
        minimalNFT.transferFrom(appAdministrator, user1, 4);
        assertEq(minimalNFT.balanceOf(user1), 2);
        ...
        vm.stopPrank();
        vm.startPrank(user1);
        minimalNFT.transferFrom(user1, user2, 3);
        assertEq(minimalNFT.balanceOf(user2), 1);
        assertEq(minimalNFT.balanceOf(user1), 1);
        switchToRuleAdmin();
        ...
        minimalNFT.transferFrom(user1, user3, 4);

        ///make sure the maximum rule fail results in revert
        vm.stopPrank();
        vm.startPrank(appAdministrator);
        // user1 mints to 6 total (limit)
        minimalNFT.safeMint(user1); /// Id 6
        minimalNFT.safeMint(user1); /// Id 7
        minimalNFT.safeMint(user1); /// Id 8
        minimalNFT.safeMint(user1); /// Id 9
        minimalNFT.safeMint(user1); /// Id 10
        ...
        minimalNFT.transferFrom(user2, user1, 3);
    ...
    function testERC721_AccountMinMaxTokenBalanceBlankTag2() public {
        ...
        minimalNFT.transferFrom(appAdministrator, rich_user, 0);
        minimalNFT.transferFrom(appAdministrator, rich_user, 1);
        minimalNFT.transferFrom(appAdministrator, rich_user, 5);
        minimalNFT.transferFrom(appAdministrator, rich_user, 6);
        minimalNFT.transferFrom(appAdministrator, rich_user, 7);
        assertEq(minimalNFT.balanceOf(rich_user), 5);
        ...
        minimalNFT.transferFrom(rich_user, user1, 5);
        assertEq(minimalNFT.balanceOf(user1), 2);
        minimalNFT.transferFrom(rich_user, user1, 6);
        assertEq(minimalNFT.balanceOf(user1), 3);
    ...
    function testERC721_AccountApproveDenyOracle2() public {
        ...
        assertEq(minimalNFT.balanceOf(user1), 5);
        ...
        vm.stopPrank();
        vm.startPrank(user1);
        minimalNFT.transferFrom(user1, user2, 0);
        assertEq(minimalNFT.balanceOf(user2), 1);
        ///perform transfer that checks rule
        // This one should fail
        vm.expectRevert(0x2767bda4);
        minimalNFT.transferFrom(user1, address(69), 1);
        assertEq(minimalNFT.balanceOf(address(69)), 0);
        ...
        minimalNFT.transferFrom(user1, address(88), 3);
...
function testERC721_TokenMaxDailyTrades() public {
        ...
        minimalNFT.safeMint(user1); // tokenId = 2
        minimalNFT.safeMint(user1); // tokenId = 3
        minimalNFT.safeMint(user1); // tokenId = 4
        ...
        switchToAppAdministrator();
        applicationAppManager.addTag(address(minimalNFT), "DiscoPunk"); ///add tag
        ...
        vm.stopPrank();
        vm.startPrank(user2);
        minimalNFT.transferFrom(user2, user1, 0);
        assertEq(minimalNFT.balanceOf(user2), 0);

        // set to a tag that only allows 1 transfer
        switchToAppAdministrator();
        applicationAppManager.removeTag(address(minimalNFT), "DiscoPunk"); ///add tag
        applicationAppManager.addTag(address(minimalNFT), "BoredGrape"); ///add tag
        // perform 1 transfer
        vm.stopPrank();
        vm.startPrank(user1);
        minimalNFT.transferFrom(user1, user2, 1);
        assertEq(minimalNFT.balanceOf(user2), 1);
        vm.stopPrank();
        vm.startPrank(user2);
        // this one should fail because it is more than 1 in 24 hours
        vm.expectRevert(0x09a92f2d);
        minimalNFT.transferFrom(user2, user1, 1);
        assertEq(minimalNFT.balanceOf(user2), 1);
        ...
        minimalNFT.transferFrom(user1, user2, 2);
        vm.stopPrank();
        vm.startPrank(user2);
        // this one should fail because it is more than 1 in 24 hours
        vm.expectRevert(0x09a92f2d);
    ...
    function testMinERC721_AccountMaxTransactionValueByRiskScore() public {
        ...
        minimalNFT.safeMint(user2); // tokenId = 5
        minimalNFT.safeMint(user2); // tokenId = 6
        minimalNFT.safeMint(user2); // tokenId = 7
        ...
        switchToAppAdministrator();
        erc721Pricer.setSingleNFTPrice(address(minimalNFT), 0, 10 * ATTO);
        erc721Pricer.setSingleNFTPrice(address(minimalNFT), 1, 11 * ATTO);
        erc721Pricer.setSingleNFTPrice(address(minimalNFT), 2, 12 * ATTO);
        erc721Pricer.setSingleNFTPrice(address(minimalNFT), 3, 13 * ATTO);
        erc721Pricer.setSingleNFTPrice(address(minimalNFT), 4, 15 * ATTO);
        erc721Pricer.setSingleNFTPrice(address(minimalNFT), 5, 15 * ATTO);
        erc721Pricer.setSingleNFTPrice(address(minimalNFT), 6, 17 * ATTO);
        erc721Pricer.setSingleNFTPrice(address(minimalNFT), 7, 20 * ATTO);

        ///Transfer NFT's
        ///Positive cases
        vm.stopPrank();
        vm.startPrank(user1);
        minimalNFT.safeTransferFrom(user1, user3, 0);

        vm.stopPrank();
        vm.startPrank(user3);
        minimalNFT.safeTransferFrom(user3, user1, 0);

        vm.stopPrank();
        vm.startPrank(user1);
        minimalNFT.safeTransferFrom(user1, user2, 4);
        minimalNFT.safeTransferFrom(user1, user2, 1);

        ///Fail cases
        vm.stopPrank();
        vm.startPrank(user2);
        vm.expectRevert();
        minimalNFT.safeTransferFrom(user2, user3, 7);

        vm.expectRevert();
        minimalNFT.safeTransferFrom(user2, user3, 6);

        vm.expectRevert();
        minimalNFT.safeTransferFrom(user2, user3, 5);

        vm.stopPrank();
        vm.startPrank(user2);
        vm.expectRevert();
        minimalNFT.safeTransferFrom(user2, user3, 4);

        ///simulate price changes
        switchToAppAdministrator();
        erc721Pricer.setSingleNFTPrice(address(minimalNFT), 4, 1050 * (10 ** 16)); // in cents
        erc721Pricer.setSingleNFTPrice(address(minimalNFT), 5, 1550 * (10 ** 16)); // in cents
        erc721Pricer.setSingleNFTPrice(address(minimalNFT), 6, 11 * ATTO); // in dollars
        erc721Pricer.setSingleNFTPrice(address(minimalNFT), 7, 9 * ATTO); // in dollars
        ...
        switchToAppAdministrator();
        erc721Pricer.setSingleNFTPrice(address(minimalNFT), 5, 14 * ATTO);
        erc721Pricer.setSingleNFTPrice(address(minimalNFT), 4, 17 * ATTO);
        erc721Pricer.setSingleNFTPrice(address(minimalNFT), 6, 25 * ATTO);
    ...
    function testERC721_AccountMaxTransactionValueByRiskScoreWithPeriod() public {
        ...
        ///simulate price changes
        switchToAppAdministrator();
        erc721Pricer.setSingleNFTPrice(address(minimalNFT), 4, 1050 * (ATTO / 100)); // in cents
        erc721Pricer.setSingleNFTPrice(address(minimalNFT), 5, 1550 * (ATTO / 100)); // in cents
        erc721Pricer.setSingleNFTPrice(address(minimalNFT), 6, 11 * ATTO); // in dollars
        erc721Pricer.setSingleNFTPrice(address(minimalNFT), 7, 9 * ATTO); // in dollars
        ...
        vm.stopPrank();
        vm.startPrank(user1);
        minimalNFT.safeTransferFrom(user1, user2, 0);
        vm.expectRevert();
        minimalNFT.safeTransferFrom(user1, user2, 1);
    }
    ...
    function testERC721_AccountMinMaxTokenBalance() public {
        ...
        minimalNFT.safeMint(user2); // tokenId = 3
        minimalNFT.safeMint(user2); // tokenId = 4
        minimalNFT.safeMint(user2); // tokenId = 5

        minimalNFT.safeMint(user3); // tokenId = 6
        minimalNFT.safeMint(user3); // tokenId = 7
        minimalNFT.safeMint(user3); // tokenId = 8
        ...
        vm.stopPrank();
        vm.startPrank(user1);
        minimalNFT.safeTransferFrom(user1, user2, 0); ///User 1 has min limit of 1
        minimalNFT.safeTransferFrom(user1, user3, 1);
        assertEq(minimalNFT.balanceOf(user1), 1);

        vm.stopPrank();
        vm.startPrank(user2);
        minimalNFT.safeTransferFrom(user2, user1, 0); ///User 2 has min limit of 2
        minimalNFT.safeTransferFrom(user2, user3, 3);
        assertEq(minimalNFT.balanceOf(user2), 2);

        vm.stopPrank();
        vm.startPrank(user3);
        minimalNFT.safeTransferFrom(user3, user2, 3); ///User 3 has min limit of 3
        minimalNFT.safeTransferFrom(user3, user1, 1);
        assertEq(minimalNFT.balanceOf(user3), 3);

        /// Transfers failing (below min value limit)
        vm.stopPrank();
        vm.startPrank(user1);
        minimalNFT.safeTransferFrom(user1, rich_user, 0); ///User 1 has min limit of 1
        minimalNFT.safeTransferFrom(user1, rich_user, 1);
        vm.expectRevert(0xa7fb7b4b);
        minimalNFT.safeTransferFrom(user1, rich_user, 2);
        assertEq(minimalNFT.balanceOf(user1), 1);

        vm.stopPrank();
        vm.startPrank(user2);
        minimalNFT.safeTransferFrom(user2, rich_user, 3); ///User 2 has min limit of 2
        vm.expectRevert(0xa7fb7b4b);
        minimalNFT.safeTransferFrom(user2, rich_user, 4);
        assertEq(minimalNFT.balanceOf(user2), 2);

        vm.stopPrank();
        vm.startPrank(user3);
        vm.expectRevert(0xa7fb7b4b);
        minimalNFT.safeTransferFrom(user3, rich_user, 6); ///User 3 has min limit of 3
        assertEq(minimalNFT.balanceOf(user3), 3);
    ...
    function testERC721_AccountMinMaxTokenBalanceBlankTag() public {
        ...
        minimalNFT.safeMint(user2); // tokenId = 2
        minimalNFT.safeMint(user2); // tokenId = 3

        minimalNFT.safeMint(user3); // tokenId = 4
        minimalNFT.safeMint(user3); // tokenId = 5
        minimalNFT.safeMint(user3); // tokenId = 6
    ...
    function testERC721_AdminMinTokenBalance() public {
        ...
        minimalNFT.safeTransferFrom(ruleBypassAccount, user1, 0);
        minimalNFT.safeTransferFrom(ruleBypassAccount, user1, 1);
        /// This one fails
        vm.expectRevert();
    ...
    function testERC721_TransferVolumeRule() public {
        ...
        vm.expectRevert(0x009da0ce);
        minimalNFT.safeTransferFrom(user1, user2, 2);
    ...
    function testERC721_NFTValuationOrig() public {
        ...
        for (uint i = 0; i < 10; i++) {
            minimalNFT.safeMint(user1);
            erc721Pricer.setSingleNFTPrice(address(minimalNFT), i, 1 * ATTO);
        }
        uint256 testPrice = erc721Pricer.getNFTPrice(address(minimalNFT), 1);
        assertEq(testPrice, 1 * ATTO);
        erc721Pricer.setNFTCollectionPrice(address(minimalNFT), 1 * ATTO);
    ...
    function testERC721_AccountMaxSellSize() public {
        ...
        switchToUser();
        minimalNFT.setApprovalForAll(address(amm), true);
    ...
    function _approveTokens(DummyNFTAMM amm, uint256 amountERC20, bool _isApprovalERC721) internal {
        ...
        minimalNFT.setApprovalForAll(address(amm), _isApprovalERC721);
    }
    ...
    function _addLiquidityInBatchERC721(DummyNFTAMM amm, uint256 amount) private {
        for(uint256 i; i < amount; i++){
            minimalNFT.safeTransferFrom(appAdministrator, address(amm), i);
        }
    }

    function _testBuyNFT(uint256 _tokenId, DummyNFTAMM amm) internal {
        amm.dummyTrade(address(applicationCoin), address(minimalNFT), 10, _tokenId, true);
    }
    ...
    function _fundThreeAccounts() internal {
        ...
        for(uint i = erc721Liq / 2; i < erc721Liq / 2 + 50; i++){
            minimalNFT.safeTransferFrom(appAdministrator, user, i);
        }
        for(uint i = erc721Liq / 2 + 100; i < erc721Liq / 2 + 150; i++){
            minimalNFT.safeTransferFrom(appAdministrator, user1, i);
        }
        for(uint i = erc721Liq / 2 + 200; i < erc721Liq / 2 + 250; i++){
            minimalNFT.safeTransferFrom(appAdministrator, user2, i);
        }
    }

...
contract ProtocolApplicationHandlerTests is TestCommonFoundry {
    ...
    function test_versionUpgradeFacet() public {
        ...
        vm.expectEmit();
        emit AD1467_UpgradedToVersion(appAdministrator, "1.3.1");
        (success, ) = address(applicationCoinHandler).call(abi.encodeWithSignature("updateVersion(string)", "1.3.1"));
        assertTrue(success);

        vm.expectEmit();
        emit AD1467_UpgradedToVersion(appAdministrator, "1.3.1");
        (success, ) = address(applicationNFTHandler).call(abi.encodeWithSignature("updateVersion(string)", "1.3.1"));
        assertTrue(success);

        vm.assertEq(HandlerVersionFacet(address(applicationCoinHandler)).version(), "1.3.1");
        vm.assertEq(HandlerVersionFacet(address(applicationNFTHandler)).version(), "1.3.1");
    }

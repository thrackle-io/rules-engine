...
contract RuleProcessorDiamondTest is Test, GenerateSelectors, TestCommonFoundry {
    ...
    function testAddMinTransferRule() public {
        switchToRuleAdmin();
        uint32 index = RuleDataFacet(address(ruleStorageDiamond)).addMinimumTransferRule(address(applicationAppManager), 1000);
    ...
    function testNotPassingMinTransferRule() public {
        switchToRuleAdmin();
        uint32 index = RuleDataFacet(address(ruleStorageDiamond)).addMinimumTransferRule(address(applicationAppManager), 420);

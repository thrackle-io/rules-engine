...
contract ApplicationERC20FuzzTest is DiamondTestUtil, RuleProcessorDiamondTestUtil {
    ...
    function testBalance(uint256 _supply) public {
        applicationCoin.mint(superAdmin, _supply);
        assertEq(applicationCoin.balanceOf(superAdmin), _supply);
    }
    ...
    function testPassesMinTransferRuleFuzz(uint8 _addressIndex, uint128 _transferAmount) public {
        ...
        applicationCoin.mint(superAdmin, type(uint256).max);
    ...
    function testTokenTransferVolumeRuleFuzzCoin(uint8 _addressIndex, uint8 _period, uint24 _maxPercent) public {
        ...
        address[] memory addressList = getUniqueAddresses(_addressIndex % ADDRESSES.length, 5);
        rich_user = addressList[0];
        uint32 _index = RuleDataFacet(address(ruleStorageDiamond)).addTransferVolumeRule(address(appManager), _maxPercent, _period, Blocktime, 100_000);
        assertEq(_index, 0);
    ...
    function testTotalSupplyVolatilityFuzz(uint8 _addressIndex, uint256 amount, uint16 volLimit) public {
        ...
        applicationCoin.mint(superAdmin, initialSupply);
        ...
        applicationCoin.transfer(superAdmin, applicationCoin.balanceOf(user1));

        /// test minimum volatility limits
        vm.stopPrank();
        vm.startPrank(superAdmin);
        applicationCoin.burn(applicationCoin.balanceOf(superAdmin));
        applicationCoin.mint(superAdmin, initialSupply);

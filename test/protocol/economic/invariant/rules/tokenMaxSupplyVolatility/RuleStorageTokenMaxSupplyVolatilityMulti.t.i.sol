 ...
 */
contract RuleStorageTokenMaxSupplyVolatilityMultiTest is RuleStorageInvariantCommon {
    ...
    function setUp() public {
        ...
        for(uint i; i < 10; i++){
            ...
            actors.push(new RuleStorageTokenMaxSupplyVolatilityActor(ruleProcessor, actorAppManager));
            if(i % 2 == 0){
                vm.startPrank(appAdministrator);
                actorAppManager.addRuleAdministrator(address(actors[actors.length - 1]));
            }
        }
        actorManager = new RuleStorageTokenMaxSupplyVolatilityActorManager(actors);
        switchToRuleAdmin();
        index = RuleDataFacet(address(ruleProcessor)).addTokenMaxSupplyVolatility(address(applicationAppManager), 666, 24, uint64(block.timestamp), 0);
        ruleBefore = ERC20RuleProcessorFacet(address(ruleProcessor)).getTokenMaxSupplyVolatility(index);
        targetContract(address(actorManager));
    }

    // The total amount of rules will never decrease.
    function invariant_rulesTotalTokenMaxSupplyVolatilityNeverDecreases() public {
        uint256 total;
        for(uint i; i < actors.length; i++){
            total += actors[i].totalRules();
        }
        // index must be incremented by one to account for 0 based array
        assertLe(index+1, ERC20RuleProcessorFacet(address(ruleProcessor)).getTotalTokenMaxSupplyVolatility());
    }
    ...
    function invariant_rulesTotalTokenMaxSupplyVolatilityEqualsAppBalances() public {
        uint256 total;
        for(uint i; i < actors.length; i++){
            total += actors[i].totalRules();
        }
        console.log(total);
        console.log(ERC20RuleProcessorFacet(address(ruleProcessor)).getTotalTokenMaxSupplyVolatility());
        // adding 1 to total for the initial rule created in the setup function
        assertEq(total + 1, ERC20RuleProcessorFacet(address(ruleProcessor)).getTotalTokenMaxSupplyVolatility());
    }

    // There can be only a total of 2**32 of each rule type.
    function invariant_rulesTotalTokenMaxSupplyVolatilityLessThanMax() public {
        assertLe(ERC20RuleProcessorFacet(address(ruleProcessor)).getTotalTokenMaxSupplyVolatility(), maxRuleCount);
    }
    ...
    function invariant_TokenMaxSupplyVolatilityImmutable() public {
        NonTaggedRules.TokenMaxSupplyVolatility memory ruleAfter = ERC20RuleProcessorFacet(address(ruleProcessor)).getTokenMaxSupplyVolatility(index);

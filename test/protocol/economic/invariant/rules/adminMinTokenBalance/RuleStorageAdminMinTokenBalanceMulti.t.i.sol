 ...
 */
contract RuleStorageAdminMinTokenBalanceMultiTest is RuleStorageInvariantCommon {
    ...
    function setUp() public {
        ...
        for (uint i; i < 10; i++) {
            ...
            actors.push(new RuleStorageAdminMinTokenBalanceActor(ruleProcessor, actorAppManager));
            if (i % 2 == 0) {
        ...
        switchToRuleAdmin();
        index = TaggedRuleDataFacet(address(ruleProcessor)).addAdminMinTokenBalance(address(applicationAppManager), 222, uint64(block.timestamp + 60 days));
        ruleBefore = ERC20TaggedRuleProcessorFacet(address(ruleProcessor)).getAdminMinTokenBalance(index);
        targetContract(address(actorManager));
    }

    // The total amount of rules will never decrease.
    function invariant_rulesTotalAdminMinTokenBalanceNeverDecreases() public view {
        uint256 total;
        for (uint i; i < actors.length; i++) {
            total += actors[i].totalRules();
        }
        // index must be incremented by one to account for 0 based array
        assertLe(index + 1, ERC20TaggedRuleProcessorFacet(address(ruleProcessor)).getTotalAdminMinTokenBalance());
    }
    ...
    function invariant_rulesTotalAdminMinTokenBalanceEqualsAppBalances() public view {
        uint256 total;
        for (uint i; i < actors.length; i++) {
            total += actors[i].totalRules();
        }
        // adding 1 to total for the initial rule created in the setup function
        assertEq(total + 1, ERC20TaggedRuleProcessorFacet(address(ruleProcessor)).getTotalAdminMinTokenBalance());
    }
    ...
    function invariant_rulesTotalAdminMinTokenBalanceLessThanMax() public view {
        assertLe(ERC20TaggedRuleProcessorFacet(address(ruleProcessor)).getTotalAdminMinTokenBalance(), maxRuleCount);
    }
    ...
    function invariant_rulesTotalAdminMinTokenBalanceIncrementsByOne() public {
        uint256 previousTotal = ERC20TaggedRuleProcessorFacet(address(ruleProcessor)).getTotalAdminMinTokenBalance();
        // not incrementing previousTotal by one due to zero based ruleId
        assertEq(previousTotal, TaggedRuleDataFacet(address(ruleProcessor)).addAdminMinTokenBalance(address(applicationAppManager), 222, uint64(block.timestamp + 60 days)));
    }

    // Rules can never be modified.
    function invariant_AdminMinTokenBalanceImmutable() public view {
        TaggedRules.AdminMinTokenBalance memory ruleAfter = ERC20TaggedRuleProcessorFacet(address(ruleProcessor)).getAdminMinTokenBalance(index);

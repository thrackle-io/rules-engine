 ...
 */
contract RuleStorageAccountMaxValueByRiskScoreMultiTest is
    RuleStorageInvariantCommon
{
    ...
    function setUp() public {
        ...
        for (uint i; i < 10; i++) {
            ...
            actors.push(
                new RuleStorageAccountMaxValueByRiskScoreActor(
                    ruleProcessor,
                    actorAppManager
                )
            );
        ...
        actorManager = new RuleStorageAccountMaxValueByRiskScoreActorManager(
            actors
        );
        switchToRuleAdmin();
        index = AppRuleDataFacet(address(ruleProcessor))
            .addAccountMaxValueByRiskScore(
                ...
                createUint8Array(50),
                createUint48Array(100)
            );
        ruleBefore = ApplicationRiskProcessorFacet(address(ruleProcessor))
            .getAccountMaxValueByRiskScore(index);
        targetContract(address(actorManager));
    }

    // The total amount of rules will never decrease.
    function invariant_rulesTotalAccountMaxValueByRiskScoreNeverDecreases()
        public
        view
    {
        uint256 total;
        ...
        assertLe(
            index + 1,
            ApplicationRiskProcessorFacet(address(ruleProcessor))
    ...
    function invariant_rulesTotalAccountMaxValueByRiskScoreEqualsAppBalances()
        public
        view
    {
        uint256 total;
    ...
    function invariant_rulesTotalAccountMaxValueByRiskScoreLessThanMax()
        public
        view
    {
        assertLe(
    ...
    function invariant_rulesTotalAccountMaxValueByRiskScoreIncrementsByOne()
        public
    {
        uint256 previousTotal = ApplicationRiskProcessorFacet(
    ...
    function invariant_AccountMaxValueByRiskScoreImmutable() public view {
        AppRules.AccountMaxValueByRiskScore
            memory ruleAfter = ApplicationRiskProcessorFacet(
                address(ruleProcessor)
            ).getAccountMaxValueByRiskScore(index);

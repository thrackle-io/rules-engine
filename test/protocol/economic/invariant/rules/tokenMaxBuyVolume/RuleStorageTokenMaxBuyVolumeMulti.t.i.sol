...
contract RuleStorageTokenMaxBuyVolumeMultiTest is RuleStorageInvariantCommon {
    ...
    function setUp() public {
        ...
        for(uint i; i < 10; i++){
            ...
            actors.push(new RuleStorageTokenMaxBuyVolumeActor(ruleProcessor, actorAppManager));
            if(i % 2 == 0){
        ...
        switchToRuleAdmin();
        index = RuleDataFacet(address(ruleProcessor)).addTokenMaxBuyVolume(address(applicationAppManager), 5000, 24, 0xffffffffffff, uint64(block.timestamp));
        ruleBefore = ERC20RuleProcessorFacet(address(ruleProcessor)).getTokenMaxBuyVolume(index);
        targetContract(address(actorManager));
    }

    // The total amount of rules will never decrease.
    function invariant_rulesTotalTokenMaxBuyVolumeNeverDecreases() public {
        uint256 total;
        for(uint i; i < actors.length; i++){
            total += actors[i].totalRules();
        }
        // index must be incremented by one to account for 0 based array
        assertLe(index+1, ERC20RuleProcessorFacet(address(ruleProcessor)).getTotalTokenMaxBuyVolume());
    }
    ...
    function invariant_rulesTotalTokenMaxBuyVolumeEqualsAppBalances() public {
        uint256 total;
        for(uint i; i < actors.length; i++){
            total += actors[i].totalRules();
        }
        console.log(total);
        console.log(ERC20RuleProcessorFacet(address(ruleProcessor)).getTotalTokenMaxBuyVolume());
        // adding 1 to total for the initial rule created in the setup function
        assertEq(total + 1, ERC20RuleProcessorFacet(address(ruleProcessor)).getTotalTokenMaxBuyVolume());
    }

    // There can be only a total of 2**32 of each rule type.
    function invariant_rulesTotalTokenMaxBuyVolumeLessThanMax() public {
        assertLe(ERC20RuleProcessorFacet(address(ruleProcessor)).getTotalTokenMaxBuyVolume(), maxRuleCount);
    }
    ...
    function invariant_rulesTotalTokenMaxBuyVolumeIncrementsByOne() public {
        uint256 previousTotal = ERC20RuleProcessorFacet(address(ruleProcessor)).getTotalTokenMaxBuyVolume();
        // not incrementing previousTotal by one due to zero based ruleId
        assertEq(previousTotal, RuleDataFacet(address(ruleProcessor)).addTokenMaxBuyVolume(address(applicationAppManager), 5000, 24, 0xffffffffffff, uint64(block.timestamp)));
    }
    // Rules can never be modified.
    function invariant_TokenMaxBuyVolumeImmutable() public {
        NonTaggedRules.TokenMaxBuyVolume memory ruleAfter = ERC20RuleProcessorFacet(address(ruleProcessor)).getTokenMaxBuyVolume(index);

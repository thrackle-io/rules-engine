 ...
 */
contract RuleStorageTokenMaxTradingVolumeMultiTest is
    RuleStorageInvariantCommon
{
    ...
    function setUp() public {
        ...
        for (uint i; i < 10; i++) {
            ...
            actors.push(
                new RuleStorageTokenMaxTradingVolumeActor(
                    ruleProcessor,
                    actorAppManager
                )
            );
        ...
        switchToRuleAdmin();
        index = RuleDataFacet(address(ruleProcessor)).addTokenMaxTradingVolume(
        ...
        ruleBefore = ERC20RuleProcessorFacet(address(ruleProcessor))
            .getTokenMaxTradingVolume(index);
        targetContract(address(actorManager));
    }

    // The total amount of rules will never decrease.
    function invariant_rulesTotalTokenMaxTradingVolumeNeverDecreases()
        public
        view
    {
        uint256 total;
    ...
    function invariant_rulesTotalTokenMaxTradingVolumeEqualsAppBalances()
        public
        view
    {
        uint256 total;
    ...
    function invariant_rulesTotalTokenMaxTradingVolumeLessThanMax()
        public
        view
    {
        assertLe(
    ...
    function invariant_rulesTotalTokenMaxTradingVolumeIncrementsByOne() public {
        ...
        assertEq(
            previousTotal,
            RuleDataFacet(address(ruleProcessor)).addTokenMaxTradingVolume(
                address(applicationAppManager),
                666,
                24,
                uint64(block.timestamp),
    ...
    function invariant_TokenMaxTradingVolumeImmutable() public view {
        NonTaggedRules.TokenMaxTradingVolume

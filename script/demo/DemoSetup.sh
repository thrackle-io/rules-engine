...
echo $APP_ADMIN
echo $APP_ADMIN_PRIVATE_KEY
echo $APPLICATION_ERC20_1

cast send $APPLICATION_ERC20_1 "mint(address,uint256)" $APP_ADMIN 10000000000000000000000000000000 --private-key $APP_ADMIN_PRIVATE_KEY --from $APP_ADMIN --rpc-url $ETH_RPC_URL
...
cast send $APPLICATION_APP_MANAGER "addRuleAdministrator(address)" $APP_ADMIN --private-key $APP_ADMIN_PRIVATE_KEY --rpc-url $ETH_RPC_URL
...
cast send $ORACLE_1_HANDLER "addAddressToApprovedList(address)" $APP_ADMIN --private-key $APP_ADMIN_PRIVATE_KEY --rpc-url $ETH_RPC_URL
...
cast send $APPLICATION_ERC20_1_HANDLER "setAccountMinMaxTokenBalanceId(uint8[], uint32)" [0] 0 --private-key $APP_ADMIN_PRIVATE_KEY --rpc-url $ETH_RPC_URL --from $APP_ADMIN
...
ECHO "export APP_ADMIN_PRIVATE_KEY=$APP_ADMIN_PRIVATE_KEY"

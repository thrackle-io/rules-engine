...
/**
 * @title Tagged Rules Diamond Library Contract
 * @author @ShaneDuncan602 @oscarsernarosero @TJ-Everett
 * @dev Library stores facets and selectors for Tagged Rules Module
 * @notice Library for Tagged Rules
 */
library TaggedRuleProcessorDiamondLib {
    bytes32 constant DIAMOND_CUT_STORAGE_POSITION = keccak256("diamond-cut.storage");
    bytes32 constant PROTOCOL_ADDRESSES = keccak256("protocol.addresses-data.storage");

    /**
     * @dev Function for position of rules. Every rule has its own storage.
     * @return ds Data storage for Tagged Rule Processor Storage
     */
    function s() internal pure returns (TaggedRuleProcessorsStorage storage ds) {
    ...
    function ruleDataStorage() internal pure returns (RuleDataStorage storage ds) {
        bytes32 position = PROTOCOL_ADDRESSES;
        assembly {
            ds.slot := position
        }
    }
    ...
    function addFunctions(address _facetAddress, bytes4[] memory _functionSelectors) internal {
        ...
        TaggedRuleProcessorsStorage storage ds = s();

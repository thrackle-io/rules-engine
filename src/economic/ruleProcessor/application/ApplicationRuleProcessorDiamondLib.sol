...
struct ApplicationRuleDataStorage {
    address ruleDiamondAddress;
}
...
/**
 * @title Access Action Library Contract
 * @notice This library provides the common data structures for use in other contracts.
 * @dev Used by various contracts for definition of actions and whether they are considered a transaction
 * @author @ShaneDuncan602, @oscarsernarosero, @TJ-Everett
 */
library ApplicationRuleProcessorDiamondLib {
    ...
    function isTransaction(uint256 _action) internal pure returns (bool) {
        if (_action != uint256(ApplicationRuleProcessorDiamondLib.ActionTypes.INQUIRE)) {
            return true;
        }
        return false;
    }
     ...
     * @return ds Data storage for Application Rule Processor Storage
     */
    ...
    function applicationStorage() internal pure returns (ApplicationRuleDataStorage storage storageStruct) {
        bytes32 position = APPLICATION_DATA_POSITION;
        assembly {
